...
<div class="pageheader">
  <h2><i class="fa fa-user"></i> Manage Organisations</h2>
...
<div class="row">
      <form class="form-horizontal" id="organisationForm" role="form" method="POST" action="{!! url('organisation/delete') !!}">
	          ...
	          <table class="table" id="class_table2">
	              ...
	              <tbody>
	              		@foreach($organisations as $organisation)
	              			<input id="organisationid" type="hidden" name="organisationid" value="{{$organisation->id}}"/>
	              			<tr>
		                   		<td>{{$organisation->name}}</td> 
		                   		<td>{{$organisation->registrationcity}}</td>      
		                   		<td>{{$organisation->contactphone}}</td>      
		                   		<td>{{$organisation->fax}}</td>      
		                   		<td>{{$organisation->email}}</td>      
		                   		<td>{{$organisation->website}}</td>      
		                   		<td class="table-action">
			                      <a href="{!!url('admin/edit_organisation/'.$organisation->id)!!}"><i class="fa fa-pencil"></i></a>
			                      <a href="#" onclick="doDelete('{{$organisation->id}}')" class="delete-row{{$organisation->id}}"><i class="fa fa-trash-o"></i></a>
			                    </td>
	                   		</tr>  
	                    @endforeach
	              </tbody>
			...
			'buttons'	: {
				'Yes'	: {
					...
					'action': function(){		
						...
						jQuery('#id').val(id);
						$( '#organisationForm' ).ajaxForm(options).submit();						
					}
				},
				'No'	: {
					'class'	: 'gray',

...
<div class="pageheader">
  <h2><i class="fa fa-user"></i> Manage Attributes</h2>
  <div class="breadcrumb-wrapper">
    <span class="label">You are here:</span>
    <ol class="breadcrumb">
      <li><a href="{{URL::route('admin/dashboard')}}">{{Config::get('app.title')}}</a></li>
      <li class="active">All Attributes</li>
    </ol>
  </div>
</div>
...
<div class="row">
      <form class="form-horizontal" id="attributesForm" role="form" method="POST" action="{!! url('attribute/delete') !!}">
	          ...
	          <table class="table" id="class_table2">
	              <thead>
	                 <tr>
						...
						<th>Attribute character type</th>
						<th>Attribute value</th>
						<th>Attribute unit value</th>
	                    <th></th>
	                 </tr>
	              </thead>
	              <tbody>
	              		@foreach($attributes as $attribute)
	              			<input id="productid" type="hidden" name="productid" value="{{$attribute->id}}"/>
	              			<tr>
		                   		<td>{{$attribute->attributename}}</td> 
		                   		<td>{{$attribute->parentattribute}}</td>     
		                   		<td>{{$attribute->attributecharactertype}}</td>          
		                   		<td>{{$attribute->attributevalue}}</td>          
		                   		<td>{{$attribute->attributeunitvalue}}</td>          
		                   		<td class="table-action">
			                      <a href="{!!url('admin/edit_attribute/'.$attribute->id)!!}"><i class="fa fa-pencil"></i></a>
			                      <a href="#" onclick="doDelete('{{$attribute->id}}')" class="delete-row{{$attribute->id}}"><i class="fa fa-trash-o"></i></a>
			                    </td>
	                   		</tr>  
	                    @endforeach
	              </tbody>
			...
			'buttons'	: {
				'Yes'	: {
					...
					'action': function(){		
						...
						jQuery('#id').val(id);
						$( '#attributesForm' ).ajaxForm(options).submit();						
					}
				},
				'No'	: {
					'class'	: 'gray',

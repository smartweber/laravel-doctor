...
<div class="pageheader">
  <h2><i class="fa fa-user"></i> Manage Events</h2>
...
<div class="row">
      <form class="form-horizontal" id="eventsForm" role="form" method="POST" action="{!! url('event/delete') !!}">
	          ...
	          <table class="table" id="class_table2">
	              ...
	              <tbody>
	              		@foreach($events as $event)
	              			<input id="productid" type="hidden" name="productid" value="{{$event->id}}"/>
	              			<tr>
		                   		<td>{{$event->name}}</td> 
		                   		<td>{{$event->startdate}}</td>          
		                   		<td>{{$event->enddate}}</td>          
		                   		<td class="table-action">
			                      <a href="{!!url('admin/edit_event/'.$event->id)!!}"><i class="fa fa-pencil"></i></a>
			                      <a href="#" onclick="doDelete('{{$event->id}}')" class="delete-row{{$event->id}}"><i class="fa fa-trash-o"></i></a>
			                    </td>
	                   		</tr>  
	                    @endforeach
	              </tbody>
			...
			'buttons'	: {
				'Yes'	: {
					...
					'action': function(){		
						...
						jQuery('#id').val(id);
						$( '#eventsForm' ).ajaxForm(options).submit();						
					}
				},
				'No'	: {
					'class'	: 'gray',
